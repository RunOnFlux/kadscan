...
export function useSearch() {
  ...
  const searchImpl = async (value: string) => {
    ...
    try {
          ...
          try {
            ...
            if (height >= 0 && height <= 20000000) {
              ...
              if (blockResponse.data?.blocksFromHeight?.edges) {
                results.blocks = blockResponse.data.blocksFromHeight.edges.map(
                  (edge: any) => {
                    return {
                      ...
                      transactionsCount:
                        edge.node.transactions?.totalCount || 0,
      ...
      if (
        (shouldSearchAll || data.filter.value === "transactions") &&
        likelyRequestKeyRegex.test(value)
      ) {
        try {
          ...
          if (txResponse.data?.transaction) {
            ...
            if (transaction.result?.badResult) {
              resultString = `{"status":"error","badResult":${JSON.stringify(
                transaction.result.badResult
              )}}`;
            } else if (transaction.result?.goodResult) {
              resultString = `{"status":"success","goodResult":${JSON.stringify(
              ...
              results.addresses = [
                {
                  ...
                  totalBalance:
                    fungibleResponse.data.fungibleAccount.totalBalance,
                },
              ];
            }
          } catch (error) {
      ...
      if (
        (shouldSearchAll || data.filter.value === "modules") &&
        looksLikeModule
      ) {
        ...
        (async (currentQuery: string) => {
          try {
            ...
            for (let i = 0; i <= 19; i++) {
              pactQuery.push({
                code: `(describe-module \"${currentQuery}\")`,
                chainId: String(i),
              });
            }
  ...
  const addToHistory = (query: string, type?: string | null) => {
    ...
    const existingIndex = data.history.findIndex(
      (h: { query: string }) => h.query === trimmed
    );
  ...
  async function shouldRedirectBeforeSearch(searchTerm: string) {
    ...
    // 1. Token symbol exact match (case-insensitive, supports leading $) - Highest Priority
    {
      ...
      if (norm.length > 0) {
        const matches = staticTokens.filter(
          (t) => (t.symbol || "").toLowerCase() === norm
        );
        if (matches.length > 0) {
    ...
    if (/^\d+$/.test(searchTerm)) {
      ...
      if (height >= 0 && height <= 20000000) {
        try {
          ...
          const firstEdge =
            blockResponse?.data?.blocksFromHeight?.edges?.[0]?.node;
          if (firstEdge) {
            data.precheck.blockHeightInfo = {
              exists: true,
              chainId: firstEdge.chainId,
              canonical: firstEdge.canonical,
            } as any;
            return {
              type: "blocks",
              chainId: firstEdge.chainId,
    ...
    if (looksLikeModule) {
      try {
        ...
        for (let i = 0; i <= 19; i++) {
          pactQuery.push({
            code: `(describe-module \"${searchTerm}\")`,
            chainId: String(i),
          });
        }
        const moduleResp: any = await $fetch("/api/graphql", {
    ...
    if ((searchTerm || "").length >= 4 && (searchTerm || "").length <= 15) {
      try {
        ...
        const hasAny =
          (codeCheck?.data?.transactionsByPactCode?.edges || []).length > 0;
        if (hasAny) {
  ...
  function shouldRedirect() {
    ...
    if (
      data?.searched?.transactions &&
      data?.searched?.transactions?.length === 1
    ) {
    ...
    if (data?.searched?.blocks && data?.searched?.blocks?.length === 1) {
      ...
      const url =
        block.canonical === false ? `${blockUrl}?canonical=false` : blockUrl;
      router.push(url);
      return true;
    }
  ...
  const handleKeyDown = async (event: KeyboardEvent) => {
    ...
    const redirectToFirstResult = () => {
      ...
      if (items.blocks && items.blocks.length > 0) {
        ...
        const url =
          b.canonical === false ? `${baseUrl}?canonical=false` : baseUrl;
        router.push(url);
        return true;
      }
    ...
    {
      const historyType =
        redirectInfo?.type === "token" ? "tokens" : redirectInfo?.type ?? null;
    ...
    if (redirectInfo) {
      if (redirectInfo.type === "blocks") {
        ...
        const url =
          redirectInfo.canonical === false
            ? `${baseUrl}?canonical=false`
            : baseUrl;
        router.push(url);
        cleanup();
        return;
